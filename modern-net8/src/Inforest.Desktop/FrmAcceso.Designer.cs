namespace Inforest.Desktop;

partial class FrmAcceso
{
    /// <summary>
    ///  Required designer variable.
    /// </summary>
    private System.ComponentModel.IContainer components = null;

    /// <summary>
    ///  Clean up any resources being used.
    /// </summary>
    /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
    protected override void Dispose(bool disposing)
    {
        if (disposing && (components != null))
        {
            components.Dispose();
        }
        base.Dispose(disposing);
    }

    #region Windows Form Designer generated code

    /// <summary>
    ///  Required method for Designer support - do not modify
    ///  the contents of this method with the code editor.
    /// </summary>
    private void InitializeComponent()
    {
        components = new System.ComponentModel.Container();
        panelHeader = new Panel();
        lblTitulo = new Label();
        lblSubtitulo = new Label();
        panelBody = new TableLayoutPanel();
        lblCaja = new Label();
        lblBaseDatos = new Label();
        txtUsuario = new TextBox();
        txtPassword = new TextBox();
        lblUsuario = new Label();
        lblPassword = new Label();
        btnIngresar = new Button();
        btnCancelar = new Button();
        btnCambiarContrasena = new Button();
        btnHuella = new Button();
        btnMarcacion = new Button();
        btnTecladoUsuario = new Button();
        btnTecladoPassword = new Button();
        lblEstado = new Label();
        lblSesion = new Label();
        panelButtons = new FlowLayoutPanel();
        panelVersion = new Panel();
        lblVersionTitulo = new Label();
        lblVersionDisponible = new Label();
        lblAcercaDe = new Label();
        timerVersiones = new System.Windows.Forms.Timer(components);
        panelHeader.SuspendLayout();
        panelBody.SuspendLayout();
        panelButtons.SuspendLayout();
        panelVersion.SuspendLayout();
        SuspendLayout();
        AutoScaleMode = AutoScaleMode.Font;
        BackColor = Color.FromArgb(248, 249, 252);
        ClientSize = new Size(520, 399);
        Controls.Add(panelVersion);
        Controls.Add(panelBody);
        Controls.Add(panelHeader);
        AcceptButton = btnIngresar;
        CancelButton = btnCancelar;
        FormBorderStyle = FormBorderStyle.FixedDialog;
        MaximizeBox = false;
        MinimizeBox = false;
        Name = "FrmAcceso";
        StartPosition = FormStartPosition.CenterScreen;
        Text = "Acceso al Sistema";
        Activated += FrmAcceso_Activated;
        Load += FrmAcceso_Load;
        panelHeader.BackColor = Color.FromArgb(234, 250, 241);
        panelHeader.Controls.Add(lblSubtitulo);
        panelHeader.Controls.Add(lblTitulo);
        panelHeader.Controls.Add(lblAcercaDe);
        panelHeader.Dock = DockStyle.Top;
        panelHeader.Location = new Point(0, 0);
        panelHeader.Name = "panelHeader";
        panelHeader.Padding = new Padding(18, 16, 18, 12);
        panelHeader.Size = new Size(520, 88);
        lblTitulo.AutoSize = true;
        lblTitulo.Font = new Font("Montserrat", 13F, FontStyle.Bold);
        lblTitulo.ForeColor = Color.FromArgb(30, 41, 71);
        lblTitulo.Location = new Point(18, 16);
        lblTitulo.Name = "lblTitulo";
        lblTitulo.Size = new Size(153, 25);
        lblTitulo.Text = "Inicio de sesión";
        lblSubtitulo.AutoSize = true;
        lblSubtitulo.Font = new Font("Poppins", 8.5F, FontStyle.Regular);
        lblSubtitulo.ForeColor = Color.FromArgb(113, 120, 131);
        lblSubtitulo.Location = new Point(20, 50);
        lblSubtitulo.Name = "lblSubtitulo";
        lblSubtitulo.Size = new Size(313, 15);
        lblSubtitulo.Text = "Acceso operativo a INFOREST por usuario, caja y terminal.";
        lblAcercaDe.AutoSize = true;
        lblAcercaDe.Cursor = Cursors.Hand;
        lblAcercaDe.Font = new Font("Segoe UI", 7.5F, FontStyle.Underline);
        lblAcercaDe.ForeColor = Color.FromArgb(113, 120, 131);
        lblAcercaDe.Location = new Point(456, 70);
        lblAcercaDe.Name = "lblAcercaDe";
        lblAcercaDe.Text = "Acerca de";
        lblAcercaDe.Click += (_, _) => MostrarAcercaDe();
        panelBody.ColumnCount = 3;
        panelBody.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 110F));
        panelBody.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100F));
        panelBody.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 38F));
        panelBody.Controls.Add(lblCaja, 0, 0);
        panelBody.Controls.Add(lblBaseDatos, 0, 1);
        panelBody.Controls.Add(lblUsuario, 0, 2);
        panelBody.Controls.Add(txtUsuario, 1, 2);
        panelBody.Controls.Add(btnTecladoUsuario, 2, 2);
        panelBody.Controls.Add(lblPassword, 0, 3);
        panelBody.Controls.Add(txtPassword, 1, 3);
        panelBody.Controls.Add(btnTecladoPassword, 2, 3);
        panelBody.Controls.Add(lblEstado, 0, 4);
        panelBody.Controls.Add(lblSesion, 0, 5);
        panelBody.Controls.Add(panelButtons, 0, 6);
        panelBody.Dock = DockStyle.Fill;
        panelBody.Location = new Point(0, 88);
        panelBody.Name = "panelBody";
        panelBody.Padding = new Padding(24, 22, 24, 18);
        panelBody.RowCount = 7;
        panelBody.RowStyles.Add(new RowStyle(SizeType.Absolute, 28F));
        panelBody.RowStyles.Add(new RowStyle(SizeType.Absolute, 28F));
        panelBody.RowStyles.Add(new RowStyle(SizeType.Absolute, 42F));
        panelBody.RowStyles.Add(new RowStyle(SizeType.Absolute, 42F));
        panelBody.RowStyles.Add(new RowStyle(SizeType.Absolute, 34F));
        panelBody.RowStyles.Add(new RowStyle(SizeType.Absolute, 34F));
        panelBody.RowStyles.Add(new RowStyle(SizeType.Absolute, 80F));
        panelBody.Size = new Size(520, 311);
        panelBody.SetColumnSpan(lblCaja, 3);
        panelBody.SetColumnSpan(lblBaseDatos, 3);
        panelBody.SetColumnSpan(lblEstado, 3);
        panelBody.SetColumnSpan(lblSesion, 3);
        panelBody.SetColumnSpan(panelButtons, 3);
        lblCaja.AutoSize = true;
        lblCaja.Font = new Font("Segoe UI", 9F, FontStyle.Bold);
        lblCaja.ForeColor = Color.FromArgb(30, 41, 71);
        lblCaja.Name = "lblCaja";
        lblCaja.Size = new Size(58, 15);
        lblCaja.Text = "CAJA 001";
        lblBaseDatos.AutoSize = true;
        lblBaseDatos.Font = new Font("Poppins", 8F, FontStyle.Regular);
        lblBaseDatos.ForeColor = Color.FromArgb(136, 145, 158);
        lblBaseDatos.Name = "lblBaseDatos";
        lblBaseDatos.Size = new Size(83, 15);
        lblBaseDatos.Text = "LOCAL : INFOREST";
        txtUsuario.Anchor = AnchorStyles.Left | AnchorStyles.Right;
        txtUsuario.BackColor = Color.FromArgb(242, 251, 247);
        txtUsuario.CharacterCasing = CharacterCasing.Upper;
        txtUsuario.Font = new Font("Poppins", 10F, FontStyle.Regular);
        txtUsuario.Name = "txtUsuario";
        txtUsuario.Size = new Size(362, 23);
        txtUsuario.KeyPress += txtUsuario_KeyPress;
        txtUsuario.Leave += txtUsuario_Leave;
        txtPassword.Anchor = AnchorStyles.Left | AnchorStyles.Right;
        txtPassword.BackColor = Color.FromArgb(242, 251, 247);
        txtPassword.Font = new Font("Poppins", 10F, FontStyle.Regular);
        txtPassword.Name = "txtPassword";
        txtPassword.PasswordChar = '*';
        txtPassword.Size = new Size(362, 23);
        txtPassword.Enter += txtPassword_Enter;
        txtPassword.KeyPress += txtPassword_KeyPress;
        lblUsuario.AutoSize = true;
        lblUsuario.Anchor = AnchorStyles.Left;
        lblUsuario.Font = new Font("Segoe UI", 9F, FontStyle.Bold);
        lblUsuario.Name = "lblUsuario";
        lblUsuario.Size = new Size(52, 15);
        lblUsuario.Text = "Usuario";
        lblPassword.AutoSize = true;
        lblPassword.Anchor = AnchorStyles.Left;
        lblPassword.Font = new Font("Segoe UI", 9F, FontStyle.Bold);
        lblPassword.Name = "lblPassword";
        lblPassword.Size = new Size(67, 15);
        lblPassword.Text = "Contraseña";
        btnIngresar.BackColor = Color.FromArgb(66, 198, 126);
        btnIngresar.FlatAppearance.BorderSize = 0;
        btnIngresar.FlatStyle = FlatStyle.Flat;
        btnIngresar.ForeColor = Color.White;
        btnIngresar.Margin = new Padding(0, 0, 12, 0);
        btnIngresar.Name = "btnIngresar";
        btnIngresar.Size = new Size(120, 30);
        btnIngresar.Text = "Ingresar";
        btnIngresar.UseVisualStyleBackColor = false;
        btnIngresar.Click += btnIngresar_Click;
        btnCancelar.BackColor = Color.White;
        btnCancelar.DialogResult = DialogResult.Cancel;
        btnCancelar.FlatAppearance.BorderColor = Color.FromArgb(226, 229, 232);
        btnCancelar.FlatStyle = FlatStyle.Flat;
        btnCancelar.Name = "btnCancelar";
        btnCancelar.Size = new Size(120, 30);
        btnCancelar.Text = "Cancelar";
        btnCancelar.UseVisualStyleBackColor = false;
        btnCancelar.Click += btnCancelar_Click;
        btnCambiarContrasena.BackColor = Color.White;
        btnCambiarContrasena.FlatAppearance.BorderColor = Color.FromArgb(226, 229, 232);
        btnCambiarContrasena.FlatStyle = FlatStyle.Flat;
        btnCambiarContrasena.Margin = new Padding(0, 0, 12, 0);
        btnCambiarContrasena.Name = "btnCambiarContrasena";
        btnCambiarContrasena.Size = new Size(120, 30);
        btnCambiarContrasena.Text = "Cambiar clave";
        btnCambiarContrasena.UseVisualStyleBackColor = false;
        btnCambiarContrasena.Click += btnCambiarContrasena_Click;
        lblEstado.AutoSize = true;
        lblEstado.ForeColor = Color.FromArgb(136, 145, 158);
        lblEstado.Name = "lblEstado";
        lblEstado.Size = new Size(125, 15);
        lblEstado.Text = "Esperando validación.";
        lblSesion.AutoSize = true;
        lblSesion.ForeColor = Color.FromArgb(136, 145, 158);
        lblSesion.Name = "lblSesion";
        lblSesion.Size = new Size(0, 15);
        panelButtons.Anchor = AnchorStyles.Right;
        panelButtons.Controls.Add(btnCancelar);
        panelButtons.Controls.Add(btnIngresar);
        panelButtons.Controls.Add(btnCambiarContrasena);
        panelButtons.Controls.Add(btnHuella);
        panelButtons.Controls.Add(btnMarcacion);
        panelButtons.FlowDirection = FlowDirection.RightToLeft;
        panelButtons.Location = new Point(109, 187);
        panelButtons.Margin = new Padding(0, 10, 0, 0);
        panelButtons.Name = "panelButtons";
        panelButtons.Size = new Size(387, 68);
        // ── Biometría: login por huella ──────────────────────────────────────
        btnHuella.BackColor = Color.FromArgb(52, 152, 219);
        btnHuella.FlatAppearance.BorderSize = 0;
        btnHuella.FlatStyle = FlatStyle.Flat;
        btnHuella.ForeColor = Color.White;
        btnHuella.Margin = new Padding(0, 0, 12, 0);
        btnHuella.Name = "btnHuella";
        btnHuella.Size = new Size(120, 30);
        btnHuella.Text = "Huella";
        btnHuella.UseVisualStyleBackColor = false;
        btnHuella.Click += btnHuella_Click;
        // ── Biometría: marcación ─────────────────────────────────────────────
        btnMarcacion.BackColor = Color.FromArgb(155, 89, 182);
        btnMarcacion.FlatAppearance.BorderSize = 0;
        btnMarcacion.FlatStyle = FlatStyle.Flat;
        btnMarcacion.ForeColor = Color.White;
        btnMarcacion.Margin = new Padding(0, 0, 12, 0);
        btnMarcacion.Name = "btnMarcacion";
        btnMarcacion.Size = new Size(120, 30);
        btnMarcacion.Text = "Marcación";
        btnMarcacion.UseVisualStyleBackColor = false;
        btnMarcacion.Click += btnMarcacion_Click;
        // ── Teclado en pantalla — usuario ────────────────────────────────────
        btnTecladoUsuario.BackColor = Color.FromArgb(236, 240, 241);
        btnTecladoUsuario.FlatAppearance.BorderColor = Color.FromArgb(200, 200, 200);
        btnTecladoUsuario.FlatStyle = FlatStyle.Flat;
        btnTecladoUsuario.ForeColor = Color.FromArgb(30, 41, 71);
        btnTecladoUsuario.Name = "btnTecladoUsuario";
        btnTecladoUsuario.Size = new Size(30, 23);
        btnTecladoUsuario.Text = "⌨";
        btnTecladoUsuario.UseVisualStyleBackColor = false;
        btnTecladoUsuario.Click += btnTecladoUsuario_Click;
        // ── Teclado en pantalla — contraseña ─────────────────────────────────
        btnTecladoPassword.BackColor = Color.FromArgb(236, 240, 241);
        btnTecladoPassword.FlatAppearance.BorderColor = Color.FromArgb(200, 200, 200);
        btnTecladoPassword.FlatStyle = FlatStyle.Flat;
        btnTecladoPassword.ForeColor = Color.FromArgb(30, 41, 71);
        btnTecladoPassword.Name = "btnTecladoPassword";
        btnTecladoPassword.Size = new Size(30, 23);
        btnTecladoPassword.Text = "⌨";
        btnTecladoPassword.UseVisualStyleBackColor = false;
        btnTecladoPassword.Click += btnTecladoPassword_Click;
        panelVersion.BackColor = Color.FromArgb(66, 198, 126);
        panelVersion.Controls.Add(lblVersionDisponible);
        panelVersion.Controls.Add(lblVersionTitulo);
        panelVersion.Dock = DockStyle.Bottom;
        panelVersion.Location = new Point(0, 369);
        panelVersion.Name = "panelVersion";
        panelVersion.Size = new Size(520, 30);
        panelVersion.Visible = false;
        lblVersionTitulo.AutoSize = true;
        lblVersionTitulo.Font = new Font("Segoe UI", 9F, FontStyle.Bold);
        lblVersionTitulo.ForeColor = Color.White;
        lblVersionTitulo.Location = new Point(12, 8);
        lblVersionTitulo.Name = "lblVersionTitulo";
        lblVersionTitulo.Size = new Size(149, 15);
        lblVersionTitulo.Text = "Actualización disponible:";
        lblVersionDisponible.AutoSize = true;
        lblVersionDisponible.ForeColor = Color.White;
        lblVersionDisponible.Location = new Point(165, 8);
        lblVersionDisponible.Name = "lblVersionDisponible";
        lblVersionDisponible.Size = new Size(0, 15);
        timerVersiones.Interval = 3000;
        timerVersiones.Tick += timerVersiones_Tick;
        panelHeader.ResumeLayout(false);
        panelHeader.PerformLayout();
        panelBody.ResumeLayout(false);
        panelBody.PerformLayout();
        panelButtons.ResumeLayout(false);
        panelVersion.ResumeLayout(false);
        panelVersion.PerformLayout();
        ResumeLayout(false);
    }

    #endregion

    private Panel panelHeader;
    private Label lblTitulo;
    private Label lblSubtitulo;
    private TableLayoutPanel panelBody;
    private Label lblCaja;
    private Label lblBaseDatos;
    private TextBox txtUsuario;
    private TextBox txtPassword;
    private Label lblUsuario;
    private Label lblPassword;
    private Button btnIngresar;
    private Button btnCancelar;
    private Button btnCambiarContrasena;
    private Button btnHuella;
    private Button btnMarcacion;
    private Button btnTecladoUsuario;
    private Button btnTecladoPassword;
    private Label lblEstado;
    private Label lblSesion;
    private FlowLayoutPanel panelButtons;
    private Panel panelVersion;
    private Label lblVersionTitulo;
    private Label lblVersionDisponible;
    private Label lblAcercaDe;
    private System.Windows.Forms.Timer timerVersiones;
}
