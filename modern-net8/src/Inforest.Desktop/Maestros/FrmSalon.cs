...
/// <summary>
/// Maestro WinForms de salones.
/// Legacy: vSalon / frmMesa.frm.
/// </summary>
public class FrmSalon : Form
{
    ...
    private List<Salon> _items = [];

    public FrmSalon(ISalonRepository repository)
    {
    ...
    {
        if (dgv.CurrentRow?.DataBoundItem is not Salon item) return;
        txtCodigo.Text = item.CodigoSalon;
    ...
    private async Task GuardarAsync()
    {
        try
        {
            ...
            {
                var nuevo = Salon.Crear(txtCodigo.Text, txtDescripcion.Text, txtResumido.Text, Environment.UserName, chkActivo.Checked);
