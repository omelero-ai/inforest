...
Private Sub Form_Load()
   wEnter = False
   Centrar Me
   If TipoOperacion = "Almacen" Or TipoOperacion = "AlmacenArticulo" Then
...
Private Sub txtCriterio_KeyUp(KeyCode As Integer, Shift As Integer)
    Select Case KeyCode
           ...
           Case 48 To 57, 65 To 90, 32, 96 To 105, 46, 8, 192
                Filtrar
    End Select
End Sub
...
Public Sub Aceptar()
   ...
   If Rs.RecordCount > 0 Then
     If TipoOperacion = "AlmacenArticulo" Then
      With FrmSeleccionProductos
            .TxtCodigo = Rs!codigo
            '.TxtCodigoBarra = IIf(IsNull(Rs!CBarra), "", Rs!CBarra)
            .txtArticulo = Rs!descripcion
            .CboUnidad.BoundText = Rs!tUnidadEntrada
            .LblStockActuales = IIf(IsNull(Rs!nStockActual), 0, Format(Rs!nStockActual, "###,###,##0.000"))
            'clr 17102011
            .tTipoIngreso = Rs!tTipoProducto
            'Jesus 15/11/06
            .TxtPrecioCompra = Rs!nPrecioCosto
            'Fin
            
            'JESUS
            .TxtPorcentajeLeyAD = Rs!nPorcentajeLeyAD
            '------02.11.2017-----------
            
            xvar1 = IIf(IsNull(Rs!nImp1 = 1), "", Rs!nImp1)
            xvar2 = IIf(IsNull(Rs!nImp2 = 1), "", Rs!nImp2)
            xVar3 = IIf(IsNull(Rs!nImp3 = 1), "", Rs!nImp3)
            If xvar1 Then: .ChkImp1.value = 1: Else: .ChkImp1.value = 0: .TxtImp1.Enabled = False
            If xvar2 Then: .ChkImp2.value = 1: Else: .ChkImp2.value = 0: .TxtImp2.Enabled = False
            If xVar3 Then: .ChkImp3.value = 1: Else: .ChkImp3.value = 0: .TxtImp3.Enabled = False
            If sPais = "005" Then
                .TxtImp1.Text = IIf(IsNull(Rs!nPorcentaje1) Or IsNull(Rs!nPorcentaje1) = 0, nPorcentaje1, Rs!nPorcentaje1)
            End If
