           ...
           Case Is = 9  'Reimpresion de Guias
               ...
               'Set RsEmision = Lib.OpenRecordset("SELECT * from vGuiaTransporte where tGuiaTransporte ='" & sDocumento & "' and tCaja='" & sCaja & "'", Cn)
                If lAlmacen And sEmpresa = "100" Then
                     'CASO CORP.FERNANDEZ
                    If BDLink = "1" Then
                        Set RsEmision = Lib.OpenRecordset("select vg.*, isnull(at.UnidadEntrada,'Sin UM') as UM from vGuiaTransporte vg left join " & "[" & sAlmacenRuta & "].[" & sAlmacenMDB & "]" & ".dbo.vPRODUCTO at on vg.tenlace= at.tCodigoProducto where  vg.tGuiaTransporte ='" & sDocumento & "' and vg.tCaja='" & sCaja & "'", Cn)
                    Else
