...
Private Sub Form_Load()
    ...
    Cn.Execute "update TDELIVERY set nConsumo = T1.nVenta FROM (SELECT SUM(dbo.DPEDIDO.nVenta)+sum((case when dbo.vproducto.lley1=1 then dbo.DPEDIDO.ncantidad * " & sValorLey1 & " else 0 end)) AS nVenta, dbo.MPEDIDO.tClienteCtaCte FROM dbo.MPEDIDO INNER JOIN dbo.DPEDIDO ON dbo.MPEDIDO.tCodigoPedido = dbo.DPEDIDO.tCodigoPedido left join dbo.vproducto on  dbo.vproducto.codigo = dbo.DPEDIDO.tcodigoproducto WHERE     (dbo.MPEDIDO.tEstadoPedido = '04') GROUP BY dbo.MPEDIDO.tClienteCtaCte) T1 Where TDELIVERY.TCODIGODELIVERY = T1.tClienteCtaCte"
