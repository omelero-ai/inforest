...
>
> Estado general: **Fase 4 IN_PROGRESS — W1-W15 completados; POS-FUNC-001..037 parcialmente migrados; baseline transversal + configuración + maestros + turno + pedidos + venta + caja + reportes + módulos POS + servicios de dominio + POS-FUNC-004 (frmSolicitudBoleta.frm MIGRATED) + POS-FUNC-021 (RFID completo MIGRATED: `FrmTarjetaProximidad` + `FrmRecargarTarjeta` + `FrmRecargarTarjetaDetalle` + `FrmMovimientoTarjetas` + BR-RFID-001..008; gap anticipo/documento abierto) + POS-FUNC-027 (frmCambio.frm MIGRATED) + POS-FUNC-028 (frmUpdateDatosPedido.frm MIGRATED) + POS-FUNC-029 (frmListaMotivos.frm MIGRATED) + POS-FUNC-019 (frmCambiarContrasenia.frm + frmPassword.frm COMPLETED) + POS-FUNC-035 (frmBusquedaRapida.frm MIGRATED) + POS-FUNC-036 (frmBusquedaDelivery.frm MIGRATED) + POS-FUNC-037 (frmClienteDelivery.frm MIGRATED) — **580 tests en verde****
...
26. **SIGUIENTE PENDIENTE:** `frmAcceso.frm` (cerrar validación funcional del acceso de `InfoRest.vbp`) o `frmDespachador.frm` (flujo operativo de despacho), manteniendo etapa actual.
27. Mantener seguimiento de gaps bloqueantes: HardKey físico, SecuGen, Epson fiscal y conectores FE/Rappi reales.
28. Completar plantillas FastReport pendientes para los handlers/reportes de Etapa 10 restantes.
