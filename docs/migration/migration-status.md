...
>
> Estado general: **Fase 4 IN_PROGRESS — W1-W15 completados; POS-FUNC-001..037 parcialmente migrados; baseline transversal + configuración + maestros + turno + pedidos + venta + caja + reportes + módulos POS + servicios de dominio + POS-FUNC-001 (`frmAcceso.frm` COMPLETED) + POS-FUNC-004 (frmSolicitudBoleta.frm MIGRATED) + POS-FUNC-021 (RFID completo MIGRATED: `FrmTarjetaProximidad` + `FrmRecargarTarjeta` + `FrmRecargarTarjetaDetalle` + `FrmMovimientoTarjetas` + BR-RFID-001..008; gap anticipo/documento abierto) + POS-FUNC-027 (frmCambio.frm MIGRATED) + POS-FUNC-028 (frmUpdateDatosPedido.frm MIGRATED) + POS-FUNC-029 (frmListaMotivos.frm MIGRATED) + POS-FUNC-019 (frmCambiarContrasenia.frm + frmPassword.frm COMPLETED) + POS-FUNC-035 (frmBusquedaRapida.frm MIGRATED) + POS-FUNC-036 (frmBusquedaDelivery.frm MIGRATED) + POS-FUNC-037 (frmClienteDelivery.frm MIGRATED) — **580 tests base + 22 tests dirigidos en verde****
...
| Código .NET 8 existente | IN_PROGRESS — 250+ archivos .cs, 580 tests verdes: baseline + seguridad + KDS + delivery + configuración + maestros + turno + pedidos + venta + caja + reportes FastReport + WinForms POS/Admin/Consultas/CajaRápida/Adición + TaxPolicy + ProductoVisibilidad + InventoryGateway + SmtpEmail + TarjetaProximidad (`FrmTarjetaProximidad`, 14 tests) + RecargaTarjeta (`FrmRecargarTarjeta` + `FrmRecargarTarjetaDetalle` + 3 tests) + RegistrarPagosMultiplesHandler + DocumentoTests (13) + NotaCredito (19 tests) + CuentaCorrienteHandlerTests (7) + CuentaCorrienteTests (8) + CentralPedidosHandlersTests (10) + RequerimientoAlmacenTests (8) + ImportacionRequerimientoHandlerTests (9) + ReportesHandlersTests (22) + CambiarDocumentoHandlerTests (8) + ActualizarDatosPedidoHandlerTests (8) + MotivoAnulacionHandlerTests (5) + SolicitudBoletaHandlersTests (2) + DeliveryHandlersTests (7) |
...
| M4-POS-FUNC-031 | Teclado Numérico + Cambio de Propina (POS-FUNC-031): `FrmNumPad` (teclado numérico modal reutilizable — migra `frmNumPad.frm`, botones 0-9 + decimal + limpiar + Enter/Esc + teclado físico) + `FrmCambioPropina` (diálogo selección propina MN/ME con NumPad, propiedades `PropinaMN/PropinaME/TipoPropina`, BR-PROPINA-001..004) — sin tests (UI pura sin lógica de aplicación) | MIGRATED — 2026-08-17 |
| M4-POS-FUNC-032 | Mapa Consulta Mesas + Cambio Estado (POS-FUNC-032): `FrmMesaConsulta` (3 modos: Visual/cambiar estado, Seleccionar/nueva mesa, Mover/transferir pedido) + `CambiarEstadoMesaHandler` + `IMesaRepository.CambiarEstadoAsync` + `MesaRepository.CambiarEstadoAsync` (UPDATE TMESA SET tEstadoMesa) + BR-MESACONSULTA-001..004 + 4 tests — **310 tests en verde** | MIGRATED — 2026-08-17 |
| M4-POS-FUNC-033 | Junta de Mesas TPEDIDOMESA (POS-FUNC-033): `FrmJuntaMesa` (gestión visual junta mesas, toggle selección, visual color-coded) + `IJuntaMesaRepository` + `JuntaMesaRepository` (transacción: UPDATE TMESA libre + DELETE TPEDIDOMESA + INSERT TPEDIDOMESA + UPDATE TMESA='06' por cada mesa) + `ObtenerMesasJuntadasHandler` + `ActualizarJuntaMesasHandler` + BR-JUNTA-001..003 — **310 tests en verde** | MIGRATED — 2026-08-17 |
| M4-POS-FUNC-034 | Correlativo de Pedidos (POS-FUNC-034): `FrmPedidoCorrelativo` (date range picker, grid de pedidos de vPedidoCorrelativo, drill-down a ítems vPedidoDetalle y docs vDocumentoAgrupado en tabs integrados) + `ObtenerPedidosCorrelativoHandler` + `ObtenerDocumentosAgrupadosPedidoHandler` + `PedidoCorrelativoVista` + `DocumentoAgrupadoVista` + `PedidoRepository.ObtenerCorrelativoAsync` + `PedidoRepository.ObtenerDocumentosAgrupadosPedidoAsync` + BR-CORRPEDIDO-001..003 — **310 tests en verde** | MIGRATED — 2026-08-17 |
| M4-POS-FUNC-021 (completion) | `FrmMovimientoTarjetas` (POS-FUNC-021 completion): vista standalone de movimientos RFID (`SELECT top 10 ... FROM TMOVIMIENTOTARJETASRFID`) — shell VB6 vacío convertido a form funcional; usa `ObtenerMovimientosTarjetaProximidadHandler` existente + BR-RFID-008 | MIGRATED — 2026-08-17 |
| M4-POS-FUNC-035 | Búsqueda Rápida Genérica (POS-FUNC-035): `FrmBusquedaRapida` (diálogo modal genérico de búsqueda con filtrado en tiempo real, selección por Enter/doble-clic, cancelación con Escape) + `BusquedaItem` + `BusquedaResultado` + BR-BUSQ-001/002/003 — sin tests (UI pura sin lógica de aplicación) | MIGRATED — 2026-08-17 |
| M4-POS-FUNC-036 | Búsqueda de Cliente Delivery (POS-FUNC-036): `FrmBusquedaDelivery` (grilla filtrable TDELIVERY+vZona, panel detalle vDelivery, panel estadísticas MPEDIDO/DPEDIDO/MDOCUMENTO, botones Nuevo/Modificar/Tiendas) + `IClienteDeliveryReadRepository` + `ClienteDeliveryReadRepository` + `ClienteDeliveryBusquedaItem` + `ClienteDeliveryDetalleBusqueda` + `EstadisticasClienteDelivery` + `TiendaDeliveryItem` + `FrmBusquedaTiendasDelivery` + 4 handlers + BR-DEL-036-001..005 — sin tests (UI pura) — **577 tests en verde** | MIGRATED — 2026-08-17 |
| M4-POS-FUNC-037 | Mantenimiento de Clientes Delivery (POS-FUNC-037): `FrmClienteDelivery` (grilla principal de `vDelivery`, filtro por columnas, ordenamiento por cabecera, navegación de registros, alta/edición modal con `NuevoDeliveryForm`, vista previa y exportación HTML) + `ObtenerClientesDeliveryListadoHandler` + `ClienteDeliveryListadoItem` + `IClienteDeliveryReadRepository.ListarMantenimientoAsync` + BR-DEL-037-001..004 + 1 test — **580 tests en verde** | MIGRATED — 2026-08-17 |
...
25. **MIGRATED:** `frmClienteDelivery.frm` (POS-FUNC-037): `FrmClienteDelivery` para mantenimiento/listado principal de clientes frecuentes con query `vDelivery`, filtro por columnas, ordenamiento por cabecera, navegación de registros, alta/edición modal mediante `NuevoDeliveryForm`, vista previa y exportación HTML + `ObtenerClientesDeliveryListadoHandler` + `ClienteDeliveryListadoItem` + `IClienteDeliveryReadRepository.ListarMantenimientoAsync` + BR-DEL-037-001..004 — **580 tests en verde**.
26. **COMPLETED:** `frmAcceso.frm` validado (POS-FUNC-001): `Form1` alineado al legacy con normalización de usuario en mayúsculas, persistencia del último usuario en `USUARIO.INI`, bootstrap de sesión con `ISessionService`, validación de licencia/configuración previa, apertura condicionada del shell por `lMCPV`, bloqueo por 3 intentos fallidos y auditoría vía `AuthService`/`AuditoriaService` + build `Inforest.Desktop` + 22 tests dirigidos (`AuthLoginIntentosTests`, `ValidarInicioPosHandlerTests`, `CambiarPasswordHandlerTests`) en verde.
27. **MIGRATED:** `frmDespachador.frm` (InfoRest.vbp): `DespachadorForm` reimplementado con filtro por rango sobre `vDespachador`, navegación de registros, asignación/desasignación de motorizado (valida empacador previo + topes `nAsignacionMotorizado`/`tMaxMotorizado`), asignación/desasignación de empacador, vista previa y exportación HTML; contratos nuevos en `IPedidoDeliveryRepository` (`vDespachador`, `vMotorizado`, `vEmpacador`, `MPEDIDO`, `TTABLA`) + handlers `AsignarMotorizadoDespachoHandler`, `DesasignarMotorizadoDespachoHandler`, `AsignarEmpacadorDespachoHandler`, `DesasignarEmpacadorDespachoHandler` + 5 tests nuevos (`DespachadorHandlersTests`).
28. **MIGRATED:** `frmPedidoDeliveryNo.frm` (InfoRest.vbp): `FrmPedidoDeliveryNo` completo para seguimiento de pedidos delivery entregados (filtro por rango sobre `vDespachador` con `tTipoPedido='02'`, `tEstadoPedido='02'`, `ISNULL(fLlegada,0)<>0`), filtro por columnas visibles, navegación de registros, detalle por pedido con `ObtenerDetallePedidoExtendidoHandler` (`vPedidoDetalle`) + nueva consulta `ObtenerPedidosSeguimientoDeliveryEntregadosHandler` + extensión `IPedidoDeliveryRepository.ObtenerSeguimientoEntregadosAsync` + BR-DEL-038 + 2 tests nuevos (`DeliveryHandlersTests`) — **320 application tests en verde**.
29. **RESUMEN INFOREST.VBP:** Forms 63/120 completados — 57 pendientes; Classes 6/10 completadas — 4 pendientes; Modules 4/18 completados — 14 pendientes; Funcionalidades POS 24/31 completadas — 7 pendientes; Reglas POS 42/52 migradas — 10 pendientes; SQL/Views/Tablas tratados 84/118 — 34 pendientes; Gaps documentados: 19.
30. **FORMULARIO ACTUAL:** `frmPedidoDeliveryNo.frm` — MIGRATED. **SIGUIENTE PENDIENTE:** `frmDocumento.frm` (emisión/documentación comercial), manteniendo la etapa actual.
31. Mantener seguimiento de gaps bloqueantes: HardKey físico, SecuGen, Epson fiscal y conectores FE/Rappi reales.
32. Completar plantillas FastReport pendientes para los handlers/reportes de Etapa 10 restantes.
