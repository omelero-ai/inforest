...
| `frmInicio.frm` (POS-FUNC-002) — Apertura de Turno | Form | `FrmAperturaTurno` + `AbrirTurnoHandler` + `AbrirTurnoCommand` + `AbrirTurnoResult` + `ObtenerUltimoTurnoHandler` + `ObtenerUltimoTurnoQuery` + `ITipoCambioRepository` + `TipoCambioDelDia` + `TipoCambioRepository` + `TurnoExistente` + `ModoConsultaTurno` + `ITurnoRepository.ObtenerUltimoTurnoAsync/GenerarCorrelativoAsync/ReAperturarAsync` + `Turno.MontoInicialME/CodigoSalon` | WinForm + Handlers + Repository + Domain | MIGRATED | `src/Inforest.Desktop/Turno/FrmAperturaTurno.cs`, `src/Inforest.Application/Turno/TurnoHandlers.cs`, `src/Inforest.Application/Turno/ITurnoRepository.cs`, `src/Inforest.Application/Turno/ITipoCambioRepository.cs`, `src/Inforest.Infrastructure/Turno/TurnoRepository.cs`, `src/Inforest.Infrastructure/Turno/TipoCambioRepository.cs` — Tests: `AbrirTurnoHandlerTests` (9) — GAP-NUMPAD-001: `FrmNumPadSimple` placeholder hasta migrar `frmNumPad.frm` |
