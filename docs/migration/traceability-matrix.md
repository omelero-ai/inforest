...
| `frmAcceso.frm` imgOpcion[2] — frmKeyBoard | Teclado en pantalla (usuario) | `FrmInputTeclado` + `btnTecladoUsuario` | Dialog + Button | COMPLETED | `modern-net8/src/Inforest.Desktop/Shared/FrmInputTeclado.cs` |
| `frmAcceso.frm` imgOpcion[3] — frmPassword | Teclado en pantalla (password) | `FrmInputTeclado` (esContrasena=true) + `btnTecladoPassword` | Dialog + Button | COMPLETED | `modern-net8/src/Inforest.Desktop/Shared/FrmInputTeclado.cs` |
| `frmAcceso.frm` imgOpcion[4] — frmVerificacionHuellaSup | Login biométrico huella 1:N | `IBiometriaService.IdentificarUsuarioAsync` + `btnHuella` | Interface + Button | MIGRATED | `modern-net8/src/Inforest.Application/Interfaces/Hardware/IBiometriaService.cs`, `modern-net8/src/Inforest.Infrastructure/Hardware/NullBiometriaService.cs` — GAP-004: BLOCKED OCX SecuGen |
| `frmAcceso.frm` imgNewProceso / Case 5 — frmVerificarHuellaMarcacion | Marcación biométrica asistencia | `IBiometriaService.CapturarHuellaAsync` + `btnMarcacion` | Interface + Button | MIGRATED | `modern-net8/src/Inforest.Application/Interfaces/Hardware/IBiometriaService.cs`, `modern-net8/src/Inforest.Infrastructure/Hardware/NullBiometriaService.cs` — GAP-004: BLOCKED OCX SecuGen |
| `frmAcceso.frm` Image1_Click — frmAbout | Acerca de | `FrmAcceso.MostrarAcercaDe` | Method | COMPLETED | `modern-net8/src/Inforest.Desktop/FrmAcceso.cs` |
| `frmInicio.frm` (POS-FUNC-002) — Apertura de Turno | Form | `FrmAperturaTurno` + `AbrirTurnoHandler` + `AbrirTurnoCommand` + `AbrirTurnoResult` + `ObtenerUltimoTurnoHandler` + `ObtenerUltimoTurnoQuery` + `ITipoCambioRepository` + `TipoCambioDelDia` + `TipoCambioRepository` + `TurnoExistente` + `ModoConsultaTurno` + `ITurnoRepository.ObtenerUltimoTurnoAsync/GenerarCorrelativoAsync/ReAperturarAsync` + `Turno.MontoInicialME/CodigoSalon` | WinForm + Handlers + Repository + Domain | COMPLETED | `src/Inforest.Desktop/Turno/FrmAperturaTurno.cs`, `src/Inforest.Application/Turno/TurnoHandlers.cs`, `src/Inforest.Application/Turno/ITurnoRepository.cs`, `src/Inforest.Application/Turno/ITipoCambioRepository.cs`, `src/Inforest.Infrastructure/Turno/TurnoRepository.cs`, `src/Inforest.Infrastructure/Turno/TipoCambioRepository.cs` — Tests: `AbrirTurnoHandlerTests` (9) — Usa `FrmNumPad` (GAP-NUMPAD-001 cerrado: `FrmNumPadSimple` eliminado, `FrmNumPad` ahora soporta `decimales` configurable) |
...
| `modPuntoVenta.bas` (`App.PrevInstance`) + `mdiPuntoVenta.frm` | Startup rule + Form MDI principal | `IAppInstanceGuard` + `AppInstanceGuard` + `Program.cs` + `FrmPuntoVenta` + `ValidarInicioCajaRapidaHandler` + `ConfiguracionCaja.lTurnoCompartido` (MDI container, toolbar 21 botones incl. _btnSalir, StatusStrip 4 paneles, menú completo Movimientos/Correlativos/Reportes/Delivery/Conexión/Ayuda, LOG_SESIONES insert+update, ActivaInicio, timers reloj/BizLink/WebApp/Conexión/Actualizador, Salir() con auditoría, InicializaMCPVAsync, CmdSalir_Click, TimerActualizador_TickAsync, FrmActualizacion, ObtenerDiaContableAsync, IniciarNoticia, LiberarDescargoVenta, flags lAlmacen/lFacturacionE/lVisaNet/lSiab/lCajaRapida/lMultilocal/lMCPV/lMultiCajero/lTurnoCompartido; **cmdOpcion2_Click (Punto de Venta): validación TC + branching lCajaRapida/lMultiCajero + AbrirVentaPos() → FrmMesaConsulta+FrmPedido; CmdMesas_Click: FrmMesaConsulta modo Visual; Program.cs: 21 formularios POS registrados en DI**) | Interface + Class + WinForm MDI | IN_PROGRESS | `modern-net8/src/Inforest.Application/Interfaces/IAppInstanceGuard.cs`, `modern-net8/src/Inforest.Application/Turno/TurnoHandlers.cs`, `modern-net8/src/Inforest.Infrastructure/Runtime/AppInstanceGuard.cs`, `modern-net8/src/Inforest.Infrastructure/Configuracion/ParametroRepository.cs`, `modern-net8/src/Inforest.Desktop/Program.cs`, `modern-net8/src/Inforest.Desktop/POS/FrmPuntoVenta.cs`, `modern-net8/src/Inforest.Desktop/POS/FrmActualizacion.cs` — GAP remanentes: frmCancelado, frmServidorEnlace, frmGuiaTransporte, frmDocElectronicoCorrelativo, frmMozoUsuario, frmConsultaSaldo, frmRepRanking, frmRepClienteDeuda (GAP-MDI-001..008) |
...
| Legacy | Tipo | .NET 8 | Tipo | Estado | Evidencia |
|---|---|---|---|---|---|
| `frmVenta.frm` | Form (POS principal 20K líneas) | `FrmVenta` (facturación) + `FrmMesaConsulta` (selección mesa) + `FrmPedido` (toma pedido) | WinForm — split por responsabilidades | MIGRATED (billing) + IN_PROGRESS (workflow POS completo) | `src/Inforest.Desktop/Ventas/FrmVenta.cs` (billing/emisión) • `src/Inforest.Desktop/Maestros/FrmMesaConsulta.cs` (selección mesa) • `src/Inforest.Desktop/Pedidos/FrmPedido.cs` (toma pedido) — GAP: combos, propiedades, motorizado, webapp-delivery, pre-cuenta no migrados |
...
| `frmDocumento.frm` (Generación) | Form | `FrmDocumento` (ítems pendientes + lista docs + generar + pagar + reimprimir + anular + NC + divisor) + `ObtenerItemsPendientesFacturacionHandler` + `ObtenerDocumentosPendientesCajaHandler` + `ReimprimirDocumentoHandler` + `IDocumentoRepository` extendido | WinForm + Handlers + Repository | MIGRATED | `src/Inforest.Desktop/Ventas/FrmDocumento.cs`, `src/Inforest.Application/Ventas/VentaHandlers.cs`, `src/Inforest.Application/Ventas/IDocumentoRepository.cs`, `src/Inforest.Infrastructure/Ventas/DocumentoRepository.cs` — BR-DOC-001..009; GAP-DOC-001..003 |
